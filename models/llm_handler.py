...
class TransformersLLM(BaseLLM):
    ...
    def generate(self, messages: List[Dict[str, str]], **kwargs) -> Iterator[str]:
        ...
        gen_params = {
            ...
            "repetition_penalty": kwargs.get("repetition_penalty", self.config.get('repetition_penalty', 1.1)),
            "do_sample": True,
        }
    ...
    def __init__(self, config: dict):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠ Warning: Ollama connection returned status {response.status_code}")
    ...
    def generate(self, messages: List[Dict[str, str]], **kwargs) -> Iterator[str]:
        ...
        payload = {
            ...
            "options": {
                "temperature": kwargs.get("temperature", self.config.get('temperature', 0.7)),
                "top_p": kwargs.get("top_p", self.config.get('top_p', 0.9)),
                "num_predict": kwargs.get("max_tokens", self.config.get('max_tokens', 512)),
            }
        }
        ...
        try:
            ...
            yield f"[Error: Could not connect to Ollama API. Is Ollama running?]"
...
def create_llm(config: dict) -> BaseLLM:
    ...
    if backend == 'transformers':
        ...
        raise ValueError(f"Unknown backend: {backend}. Choose 'transformers' or 'ollama'")
